...
namespace {
	...
	auto frameToString(const _PyInterpreterFrame& frameObject) -> string
	{
		ostringstream oss;

		auto codeObject = frameObject.f_code();
...
namespace PyExt {
	...
	EXT_COMMAND(pystack, "Prints the Python stack for the current thread, or starting at a provided PyFrameObject", "{;s,o;PyFrameObject address}")
	{
		...
		try {
			...
			auto frameObject = threadState->frame();
			// } else {
			...
			// Print each frame.
			for (; frameObject != nullptr; frameObject = frameObject->previous()) {
